		...
		<key name="{translate(@name, '_', '-')}">
			<xsl:attribute name="type">
				<xsl:choose>
					...
					<xsl:when test="@type = 'key' or @type = 'button' or @type = 'string'">s</xsl:when>
			...
			<default>
				<xsl:choose>
					...
					<xsl:when test="@type = 'key' or @type = 'button' or @type = 'string'">''</xsl:when>
